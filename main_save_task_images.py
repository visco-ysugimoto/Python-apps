...
def on_ok():
    ...
    if selected in ["Option 1", "Option 2", "Option 3"]:
        ...
        save_task_images_CamNum_selection.process_images(
            img_folder_path, folder_var.get(), save_mode, save_cam
        )
        # 選択された結果をポップアップで表示
...
def create_option1_widgets():
    ttk.Label(dynamic_frame, text="グループ番号:", font=font_medium).grid(
        row=0, column=0, padx=5, pady=5, sticky="e"
    )
    ttk.Entry(dynamic_frame, font=font_medium).grid(
        row=0, column=1, padx=5, pady=5, sticky="w"
    )
    ttk.Label(dynamic_frame, text="タスク番号:", font=font_medium).grid(
...
def create_option2_widgets():
    ...
    ttk.Label(dynamic_frame, text="ファイル選択(ziq,zit,zii):", font=font_medium).grid(
...
def create_option3_widgets():
    ...
    # フォルダ選択欄の作成
    ttk.Label(dynamic_frame, text="共有VTVフォルダ選択:", font=font_medium).grid(
        row=2, column=0, padx=5, pady=5, sticky="w", columnspan=3
    )

    folder_entry = ttk.Entry(
        dynamic_frame, textvariable=option3_folder_var, font=font_medium
    )
    ...
    folder_button = ttk.Button(
        dynamic_frame,
        text="参照",
        command=lambda: select_folder_for_var(option3_folder_var),
        width=8,
    )
...
title_separator = ttk.Separator(
    right_frame, orient="horizontal", style="Custom.TSeparator"
)
...
option1 = ttk.Radiobutton(
    option_frame,
    text="VTV9000上のタスクから(オフラインPC)",
    variable=selected_option,
    value="Option 1",
)
option2 = ttk.Radiobutton(
    option_frame,
    text="タスクファイルから(ziq, zit, zii)",
    variable=selected_option,
    value="Option 2",
)
option3 = ttk.Radiobutton(
    option_frame,
    text="VTV9000上のタスクから(共有VTV)",
    variable=selected_option,
    value="Option 3",
)
...
save_label = ttk.Label(
    save_frame, text="画像を保存するフォルダを選択", font=font_medium
)
